...
class TeamOfSeasonRankDialog extends StatefulWidget {
  const TeamOfSeasonRankDialog({
    Key? key,
  ...
  @override
  State<TeamOfSeasonRankDialog> createState() => _TeamOfSeasonRankDialogState();
}

class _TeamOfSeasonRankDialogState extends State<TeamOfSeasonRankDialog> {
  ...
  Widget build(BuildContext context) {
    final bool isCurrentUserInTop = widget.teamOfSeason.team
            .map((e) => e.userId)
    ...
    return WillPopScope(
      onWillPop: () async {
        _acknowledgeReward((widget.teamOfSeason.amountToClaim ?? 0).toInt());
        return false;
      },
      child: Dialog(
        ...
        child: SizedBox(
          ...
          child: Stack(
            children: <Widget>[
                    ...
                    children: [
                      ...
                      Align(
                        ...
                        child: Padding(
                            ...
                            child: widget.teamOfSeason.hasUnclaimedPrize &&
                                    (widget.teamOfSeason.amountToClaim ?? 0) > 0
                                ? CoinActionButton(
                                    confineInSafeArea: false,
                                    text: "teamOfTheWeekDialogCollectReward".tr(),
                                    amount: widget.teamOfSeason.amountToClaim
                                            ?.toInt() ??
                                        0,
                                    onPressed: () async {
                                      _acknowledgeReward(
                                          (widget.teamOfSeason.amountToClaim ?? 0)
                                              .toInt());
                                    },
                                  )
                                : PrimaryButton(
                                    ...
                                    onPressed: () {
                                      _acknowledgeReward(
                                          (widget.teamOfSeason.amountToClaim ??
                                              0)
                                              .toInt());
                                    })),
                      ),
              ...
              const Positioned(right: 10, child: UserCoins()),
  ...
  void _acknowledgeReward(int coins) async {
    ...
    try {
      ...
      await context.read<RankingProvider>().claimTeamOfSeasonReward();
      if (coins > 0) {
